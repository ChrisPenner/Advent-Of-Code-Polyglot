...
main() ->
	Combos = combos(?JARS),
	Sum150 = lists:sort(fun(A, B) -> length(A) < length(B) end, lists:filter(fun(X) -> lists:sum(X) =:= 150 end, Combos)),
	LeastAmnt = lists:filter(fun(X) -> length(X) =:= length(hd(Sum150)) end, Sum150),
	io:format("~p~n", [length(LeastAmnt)]).
