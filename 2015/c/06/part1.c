...
int main(int argc, char** argv)
{
    ...
    bool lights[1000][1000] = { false };  // lights 2D array
    ...
    long lights_on = 0;  // counter for lights that are on
    ...
    // Calculate no of lights that are ON
    for (i=0, lights_on=0; i<1000; i++) {
        for (j=0; j<1000; j++) {
            if (lights[i][j] == ON) {
                lights_on += 1;
            }
