...
function animateLights($lights) {
    ...
    foreach ($lights as $rowNumber => $row) {
        ...
        foreach ($row as $lightNumber => $light) {
            ...
            if (isCorner($lights,$rowNumber, $lightNumber)) {
                $newRow[] = true;
            } elseif ($light && ($neighborsOn != 2 && $neighborsOn != 3)) {
                $newRow[] = false;
...
function isCorner($lights, $row, $light) {
    if ($row == 0 && $light == 0) {
        return true;
    }

    if ($row == 0 && $light == count($lights[0]) - 1) {
        return true;
    }

    if ($row == count($lights) - 1 && $light == 0) {
        return true;
    }

    if ($row == count($lights) - 1 && $light == count($lights[0]) - 1) {
        return true;
    }
